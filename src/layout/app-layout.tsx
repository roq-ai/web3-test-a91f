...
import { getInsurers } from 'apiSdk/insurers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<InsurerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/insurers` : null),
    () => getInsurers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Insurer',
      path: isTenantUser ? `/insurers/view/${data?.[0]?.id}` : '/insurers',
      entity: 'insurer',
