...
import { getInsurerById } from 'apiSdk/insurers';
...
function InsurerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<InsurerInterface>(
    ...
    () =>
      getInsurerById(id, {
        relations: ['user', 'end_customer', 'team_member'],
      }),
  );
  ...
  const end_customerHandleCreate = async () => {
    setCreateError(null);
    try {
      await createEndCustomer({ insurer_id: id, user_id: end_customerUserId });
  ...
  const team_memberHandleCreate = async () => {
    setCreateError(null);
    try {
      await createTeamMember({ insurer_id: id, user_id: team_memberUserId });
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('insurer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/insurers/edit/${data?.id}`} passHref legacyBehavior>
              <Button
